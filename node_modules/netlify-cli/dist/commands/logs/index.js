...
const createDeprecatedFunctionCommand = (program) => {
    ...
    cmd.action((functionNames) => {
        ...
        return logAndThrowError([
            ...
            '',
            `To get logs for ${functionNames.length > 0
                ? functionNames.length === 1
                    ? 'this function'
                    : 'these functions'
                : 'all functions'} from the last 10 minutes, run:`,
            '',
            `  ${chalk.cyan(example)}`,
            '',
...
const createDeprecatedEdgeFunctionCommand = (program) => {
    const base = netlifyCommand();
    program
        .command('logs:edge-functions', { hidden: true })
        .description(`[Deprecated] Use \`${base} logs\` instead`)
        .allowUnknownOption(true)
        .action(() => logAndThrowError([
...
const createDeprecatedDeployCommand = (program) => {
    const base = netlifyCommand();
    const cmd = program
...
export const createLogsCommand = (program) => {
    ...
    return program
        ...
        .addOption(new Option('-s, --source <type...>', 'Log sources to include. Defaults to functions and edge-functions').choices([
        'functions',
        'edge-functions',
        'deploy',
    ]))
