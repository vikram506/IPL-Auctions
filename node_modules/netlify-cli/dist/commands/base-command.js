...
export default class BaseCommand extends Command {
    ...
    createHelp() {
        ...
        help.longestOptionTermLength = (command, helper) => (command.noBaseOptions === false &&
    ...
    async authenticate(tokenFromFlag) {
        ...
        if (!isInteractive()) {
            return logAndThrowError(`Authentication required. NETLIFY_AUTH_TOKEN is not set and ${chalk.cyanBright('`netlify status`')} also informs us that you need to use ${chalk.cyanBright('`netlify login --request <message>`')} as a next step.`);
        }
    ...
    async expensivelyAuthenticate() {
        ...
        if (!browserOpened && !isInteractive() && ticket.id) {
            ...
            logJson({
                ...
                agent_next_steps: 'Give the URL to the user so they can authorize. Then poll the check_command for up to ten minutes to see if the user has logged in, or wait for them to tell you and then use check_command after.',
            });
